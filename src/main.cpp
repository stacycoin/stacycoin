...
uint256 hashGenesisBlock("0x00000a2325a43ea743be1c690fce5b088a189d155c0967fb2e931f282ef4ea80"); 
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // StacyCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "StacyCoin Signed Message:\n";
...
{
    // StacyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 420000 blocks, which will occur approximately every year
    nSubsidy >>= (nHeight / 420000); // StacyCoin: 420k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // StacyCoin: every 2 min
static const int64 nTargetSpacing = 2.5 * 60; // StacyCoin: 40 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // StacyCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // StacyCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000008a150a5c403189e7e58c54d6cd9118c77d71420ba8d571fb6be6a196742");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Herbert-inspired Altcoin STACY makes its debut to rival controversial MaxCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 112;
        block.nTime    = 1392244412;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 523118;

        if (fTestNet)
        {
            block.nTime    = 1392211625;
            block.nNonce   = 179835;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x34769ade75103963d34b09aeb574a38935b912bc888461b047e865253fe41965"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // StacyCoin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            if (--nLimit <= 0)
            {
                // When this block is requested, we'll send an inv that'll make the
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StacyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StacyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StacyCoinMiner(CWallet *pwallet)
{
    printf("StacyCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running StacyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StacyCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StacyCoinMiner, pwallet));
}
